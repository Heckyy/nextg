...
function logout() {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      $.ajax({
        url:
          localStorage.getItem("data_link") + "/src/themes/proses/proses.php",
        method: "POST",
        data: data,
        type: "json",
        cache: false,
...
function change_password() {
  Swal.fire({
    ...
    preConfirm: (password) => {
      if (password == "") {
        Swal.showValidationMessage("You haven't entered a new password yet!!!");
      } else {
        Swal.fire({
          icon: "question",
