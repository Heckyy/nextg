...
class request
{
	...
	function view_request($db, $e, $library_class, $view, $page)
	{
		...
		if ($_SESSION['request'] == 1) {
			...
			$result = $db->select('tb_request', 'number_request LIKE "%' . $ubah_pencarian . '%" && tanggal LIKE "%' . $priod . '%" || cluster LIKE "%' . $ubah_pencarian . '%" && tanggal LIKE "%' . $priod . '%" || position LIKE "%' . $ubah_pencarian . '%" && tanggal LIKE "%' . $priod . '%" || code_cluster LIKE "%' . $ubah_pencarian . '%" && tanggal LIKE "%' . $priod . '%" || employee LIKE "%' . $ubah_pencarian . '%" && tanggal LIKE "%' . $priod . '%"', 'id_request', 'DESC');
...
?>
			...
			<script src="<?php echo $e; ?>/src/request/js/jsproses.js"></script>
			<div class="app-card-header p-3 main-content container-fluid">
				...
				<div class="col" align="right">
					...
					?>
						<a href="<?php echo $e; ?>/request/new" class="btn btn-default btn-sm link-new">
							New
						</a>
					<?php
					}
