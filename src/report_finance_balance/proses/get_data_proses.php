...
if (!empty($_SESSION['id_employee']) && !empty($_POST['proses'])) {
	...
	if ($_POST['proses'] == 'tarik_data') {
		...
		// $data = $db->selectpage('tb_cash_receipt_payment', $select_bank . 'tanggal BETWEEN "' . $tanggal_awal . '" AND "' . $tanggal_akhir . '" && status="1"', 'id_cash_receipt_payment', 'DESC', $startFrom, $perPage);
		...
		if (mysqli_num_rows($data) > 0) {
			...
			$date_previous_month = date('Y-m', strtotime('-1 month', strtotime($select_to)));
