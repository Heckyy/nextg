	...
	if(!empty($_POST['proses']) && !empty($_SESSION['id_employee'])){
		...
		if($proses=='new' && $_SESSION['purchasing_new']==1){
			if(!empty($_POST['supplier'])){
				...
				$cek=$db->select('tb_purchasing','tanggal LIKE "%'.$date.'%"','urut','DESC');
				...
				$cek_permintaan_detail=$db->select('tb_request_detail','number_request="'.$p['number_request'].'"','id_request_detail','ASC');
				while($cpd=mysqli_fetch_assoc($cek_permintaan_detail)){

					$acak=str_replace("=", "", base64_encode($cpd['id_request_detail']));
					...
					while($pd=mysqli_fetch_assoc($permintaan_detail)){
						...
						if(!empty($_POST['qty'.$acak]) && !empty($_POST['amount'.$acak])){
							...
							if(!empty($qty) && !empty($amount)){
								...
								$db->insert('tb_purchasing_detail','number_purchasing="'.$number.'",id_item="'.$i['id_item'].'",item="'.$i['item'].'",qty="'.$qty.'",unit="'.$pd['unit'].'",amount="'.$amount.'"');
			...
			$po = $_POST['po'];
			$cekPo=$db->select('tb_item_receipt_out','status_inv_purchasing = "1" and number_purchasing ='.'"'.$po.'"','number_purchasing','ASC');
			$jum =mysqli_num_rows($cekPo);
			echo $jum;
		...
		}else if (($proses=='cek_cancel' && !empty($_SESSION['number_purchasing']) && $_SESSION['purchasing_process']==1)){
			$cekStatus = $db->select('tb_item_receipt_out','number_purchasing='.'"'.$_SESSION['number_purchasing'].'"','number_purchasing','ASC','status_inv_purchasing');
			$resultStatus = mysqli_fetch_assoc($cekStatus);
			echo $resultStatus['status_inv_purchasing'];
			...
			if($jum>0){
				...
				while($d=mysqli_fetch_assoc($detail)){
					...
					$po=$db->select('tb_purchasing INNER JOIN tb_purchasing_detail ON tb_purchasing.number_purchasing=tb_purchasing_detail.number_purchasing','number_request="'.$c['number_request'].'" && tb_purchasing_detail.id_item="'.$d['id_item'].'" && tb_purchasing.status<2','tb_purchasing_detail.id_purchasing_detail','DESC');
...
?>
						...
						<tr>
							...
							<td align="center"><?php echo $d['qty'].' '.$d['unit']; ?></td>
							<td>
